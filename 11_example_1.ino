...
#define _DUTY_MIN 553 // servo full clockwise position (44 degree)
#define _DUTY_NEU 1476 // servo neutral position (92 degree)
...
// get a distance reading from the USS
  ...
  dist_ema = dist_ema*(1-alpha)+alpha*dist_raw;
...
if(dist_ema < 180.0) {
  ...
  else if(dist_ema < 360.0){ // if (dist_ema > 180.0 && dist_ema < 220.0)
     myservo.writeMicroseconds((_DUTY_MAX-_DUTY_MIN)*((dist_raw-180)/180)+_DUTY_MIN);
     Serial.print(myservo.read());
  }
  else {
...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < dist_ema || dist_ema > dist_max) {
